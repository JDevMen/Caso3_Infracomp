...
public class ClienteProtocoloConSeguridad {
	...
	public static void procesar(BufferedReader stdIn, BufferedReader pIn, PrintWriter pOut) throws IOException {
		...
		try {
			...
			PublicKey llavePublicaServidor = certSer.getPublicKey();
			...
			System.out.println("Se recibe el mensaje cifrado C(K_S+,K_SC)");
			if((fromServer = pIn.readLine())!= null){
				System.out.println("Respuesta del servidor: " + fromServer);
			}
			while(fromServer.length()%4!=0){
			...
			byte[] cifradoServer = DatatypeConverter.parseBase64Binary(fromServer);

			byte[] K_SC = descifrarAsimetrico(llavePrivada, algAsimetrico, cifradoServer);

			SecretKey llaveSimetrica = new SecretKeySpec(K_SC, algSimetrico);
			...
			byte[] cifradoReto = DatatypeConverter.parseBase64Binary(fromServer);

			byte[] reto = descifrarSimetrico(llaveSimetrica, cifradoReto,algSimetrico);

			String retoString = DatatypeConverter.printBase64Binary(reto);

			byte[] recifrarReto = cifrarAsimetrico(llavePublicaServidor, algAsimetrico, retoString);

			String recifrarRetoString = DatatypeConverter.printBase64Binary(recifrarReto);

			System.out.println("El reto decifrado es: " + retoString);

			pOut.println(recifrarRetoString);
			...
			byte[] idCifrado = cifrarSimetrico(llaveSimetrica, idUsuario,algSimetrico);

			String idCifradoString = DatatypeConverter.printBase64Binary(idCifrado);
			System.out.println("Se manda el id del usuario cifrado");
			pOut.println(idCifradoString);
			...
			byte[] horariomilitar = DatatypeConverter.parseBase64Binary(fromServer);

			byte[] descifrarhorariomilitar = descifrarSimetrico(llaveSimetrica, horariomilitar,algSimetrico);

			String hhmm = DatatypeConverter.printBase64Binary(descifrarhorariomilitar);
